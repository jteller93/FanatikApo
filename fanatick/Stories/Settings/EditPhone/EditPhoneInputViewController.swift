//
//  EditPhoneInputViewController.swift
//  fanatick
//
...
class EditPhoneInputViewController: ViewController {
    private struct Constant {
        static let inputViewTopMargin: CGFloat = 120
    }
    
    var viewModel = EditPhoneInputViewModel()
    ...
    override func addObservables() {
        ...
        viewModel.success.subscribe(onNext: { [weak self] (_) in
            self?.navigationController?.pushViewController(EditPhoneVerifyViewController(), animated: true)
        }).disposed(by: disposeBag)
    }
}
